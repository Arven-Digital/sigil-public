...
const API_URL = (process.env.NEXT_PUBLIC_API_URL ?? "https://api.sigil.codes").trim();
      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="text-center space-y-4 mb-16">
          ...
          <p className="text-lg text-white/40 max-w-2xl mx-auto">
            ...
            3-tier co-signing, policy engine, and session keys — all via REST API.
          </p>
        </div>
        ...
        <div className="space-y-8">
          <div className="bg-gradient-to-r from-[#00FF88]/10 to-[#00FF88]/10 border border-[#00FF88]/30 rounded-xl p-8">
            <h2 className="text-xl font-bold mb-3">🤖 Conversational Setup</h2>
            <p className="text-white/40 mb-4">
              The recommended flow: your agent asks the Origin Wallet holder a few questions, collects their preferences, then handles the deployment. No technical knowledge needed from the user.
            </p>
...
// Step 2: Walk through questions with the Origin Wallet holder
...
// Step 4: Present contract calls to Origin Wallet for signing`}
            </pre>
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-white/40 mb-3 ml-10">
                  Use an API key (generated by the Origin Wallet holder from the dashboard) or a delegation signature.
                </p>
...
const auth = await fetch("${API_URL}/v1/agent/auth/delegation", {
  ...
  body: JSON.stringify({
    ...
    signature: "0x...",      // EIP-712 signed by Origin Wallet
...
const { token } = await auth.json();`}
              ...
              <div>
                ...
                <p className="text-sm text-white/40 mb-3 ml-10">
                  Use the guided setup endpoint — it returns all contract calls you need.
                </p>
                <pre className="bg-[#0A0A0A] rounded-lg p-4 text-sm font-mono overflow-x-auto ml-10">
{`const setup = await fetch("${API_URL}/v1/agent/setup", {
  method: "POST",
    ...
    dailyLimit: "2000000000000000000", // 2 AVAX
          ...
          <div className="bg-white/[0.02] border border-white/5 rounded-xl p-8">
            ...
            <div className="space-y-4">
              ...
              <Endpoint method="GET" path="/v1/agent/setup/wizard" desc="Conversational setup guide — questions, defaults, example dialogue" />
              ...
              <Endpoint method="POST" path="/v1/agent/setup" desc="Guided all-in-one setup — returns all contract calls needed" />
          ...
          <div className="bg-white/[0.02] border border-white/5 rounded-xl p-8">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <h3 className="font-semibold text-[#00FF88] text-sm">✓ Agents CAN</h3>
                <ul className="text-sm text-white/40 space-y-1">
                  <li>• Deploy new wallets</li>
                  <li>• Configure policies & limits</li>
                  <li>• Manage target/function whitelists</li>
                  <li>• Create session keys</li>
                  <li>• View wallet status & audit logs</li>
                  <li>• Set token allowance policies</li>
                </ul>
              </div>
              <div className="space-y-2">
                <h3 className="font-semibold text-[#F04452] text-sm">✗ Agents CANNOT</h3>
                <ul className="text-sm text-white/40 space-y-1">
                  <li>• Withdraw or transfer funds</li>
                  <li>• Freeze or unfreeze Sigil Wallets</li>
                  <li>• Transfer Origin Wallet ownership</li>
                  <li>• Upgrade contract implementation</li>
                  <li>• Execute timelock transactions</li>
                  <li>• Modify recovery guardians</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/[0.02] border border-white/5 rounded-xl p-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { name: "Ethereum", chainId: 1, factory: "0x20f926bd5f416c875a7ec538f499d21d62850f35" },
                { name: "Polygon", chainId: 137, factory: "0x483D6e4e203771485aC75f183b56D5F5cDcbe679" },
                { name: "Avalanche", chainId: 43114, factory: "0x86e85de25473b432dabf1b9e8e8ce5145059b85b" },
                { name: "Base", chainId: 8453, factory: "0x5729291ed4c69936f5b5ace04dee454c6838fd50" },
                { name: "Arbitrum One", chainId: 42161, factory: "0x2f4dd6db7affcf1f34c4d70998983528d834b8f6" },
                { name: "0G Mainnet", chainId: 16661, factory: "0x8bAD12A489338B533BCA3B19138Cd61caA17405F" },
              ].map(c => (
                <div key={c.chainId} className="p-4 bg-[#0A0A0A] rounded-lg border border-white/5">
                  <h3 className="font-semibold">{c.name}</h3>
                  <p className="text-sm text-white/40 mt-1">Chain ID: {c.chainId}</p>
                  <code className="text-xs text-[#00FF88] break-all">Factory: {c.factory}</code>
                </div>
              ))}
            </div>
          </div>
