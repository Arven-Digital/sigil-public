 ...
 */
contract SigilAccountV12Test is BaseAccount, Initializable, ReentrancyGuard, UUPSUpgradeable, IERC1271, IERC165 {
    ...
    mapping(address => bool) public allowedERC1271Callers; // V12: scoped agent ERC-1271
    ...
    uint256 internal constant DEFAULT_TRANSFER_DELAY = 5 minutes; // TEST: reduced from 24h
    ...
    event ERC1271CallerUpdated(address indexed caller, bool allowed);
    ...
    uint256 internal constant UPGRADE_DELAY = 5 minutes; // TEST: reduced from 24h
    ...
    function _isAuthorizedSigner(address signer) internal view returns (bool) {
        ...
        // V12: Scoped ERC-1271 — agent key allowed ONLY for whitelisted protocol callers.
        // msg.sender is the contract calling isValidSignature (e.g. Polymarket Exchange).
        // Owner must explicitly whitelist each protocol via setAllowedERC1271Caller().
        if (signer == agentKey && !isFrozen && allowedERC1271Callers[msg.sender]) return true;

        // All other cases (unscoped agent, session keys) remain blocked.
        // Agent and session keys must operate through ERC-4337 UserOps for full policy enforcement.
        return false;
    }
